...
The attributes for characteristics and descriptors are allocated from the attribute pool.
  * @param Service_UUID_Type UUID type: 0x01 = 16 bits UUID while 0x02 = 128 bits UUID
...
attribute and return the handle of this attribute in Included_handle.
  ...
  * @param Include_UUID_Type UUID type: 0x01 = 16 bits UUID while 0x02 = 128 bits UUID
...
/**
  ...
  * @param Char_UUID_Type UUID type: 0x01 = 16 bits UUID while 0x02 = 128 bits UUID
  * @param Char_UUID See @ref Char_UUID_t
  * @param Char_Value_Length Maximum length of the characteristic value.
  * @param Char_Properties Characteristic Properties (Volume 3, Part G, section 3.3.1.1 of Bluetooth Specification 5.0)
  * Flags:
  - 0x00: CHAR_PROP_NONE
...
/**
  ...
  * @param Char_Desc_Uuid_Type UUID type: 0x01 = 16 bits UUID while 0x02 = 128 bits UUID
...
The responses are given through the @ref aci_att_read_by_type_resp_event event.
  * @param Connection_Handle Connection handle for which the command is given.
  * Values:
  - 0x0000 ... 0x0EFF
  * @param Start_Handle First requested handle number
...
/**
  * @brief This command sends a Read By Type Request packet to the server in order to read the value attribute of the characteristics specified by the UUID.
When the procedure is completed, an @ref aci_gatt_proc_complete_event event is generated.
Before procedure completion, the response packet is given through one @ref aci_gatt_disc_read_char_by_uuid_resp_event event per reported attribute.
Note: the number of bytes of a value reported by @ref aci_gatt_disc_read_char_by_uuid_resp_event event can not exceed BLE_EVT_MAX_PARAM_LEN - 7 i.e. 248 bytes for default value of BLE_EVT_MAX_PARAM_LEN.
  * @param Connection_Handle Connection handle for which the command is given.
  * Values:
  - 0x0000 ... 0x0EFF
